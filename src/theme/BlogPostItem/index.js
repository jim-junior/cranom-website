...
function BlogPostItem(props) {
        ...
        <div className="avatar margin-vert--md">
          ...
          <div className="avatar__intro">
            {author && <>
                ...
                <small className={clsx("avatar__subtitle", styles.as)}>{authorTitle}</small>
              </>}
          </div>
        </div>
      </header>;
  ...
  return <>
      ...
      <article className={!isBlogPostPage ? clsx('margin-bottom--xl', styles.cover) : undefined}>
        {renderPostHeader()}
        <section className={clsx("markdown", styles.md)}>
        ...
        {(tags.length > 0 || truncated) && <footer className="row margin-vert--lg">
          ...
          }) => <Link key={tagPermalink} className={clsx("margin-horiz--sm", styles.tags)} to={tagPermalink}>
                    {label}
                  </Link>)}
              </div>}
